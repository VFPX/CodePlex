...
DEFINE CLASS cHtmlHelpCreator AS Session
	...
	FUNCTION Init
		...
		SELECT vi.expanded AS funcname, CAST('<a href="/pages/' + ALLTRIM(vi.expanded) + '.html">' + ALLTRIM(expanded) + '</a>' AS C(254)) AS funclink FROM vi ;
	...
	FUNCTION CreateCHM
		...
		ELSE
			...
			SELECT htmlhelp
			SCAN
				STRTOFILE(htmlhelp.html, THIS.cHtmlPath + FORCEEXT(ALLTRIM(htmlhelp.funcname), 'html'), 0)	
			ENDSCAN
	...
	FUNCTION CreateFunctionPage
		...
		IF !EMPTY(vi.groupid) OR !EMPTY(vi.apifuncs)
			...
			IF !EMPTY(vi.groupid)
				...
				SELECT vigroup
				SCAN
					lcHtml = lcHtml + '<a href="/pages/' + ALLTRIM(vigroup.expanded) + '.html">' + ALLTRIM(vigroup.expanded) + '</a><br />' + CRLF
				ENDSCAN
...
TEXT TO m.lcEntry TEXTMERGE NOSHOW
		<LI> <OBJECT type="text/sitemap">
			...
			<param name="Local" value="pages\<<ALLTRIM(vs.expanded)>>.html">
			</OBJECT>
ENDTEXT
...
TEXT TO m.lcSiteMapGroup ADDITIVE TEXTMERGE NOSHOW
	<LI> <OBJECT type="text/sitemap">
		...
		<param name="Local" value="pages\group<<vg.groupid>>.html">
		</OBJECT>
	<UL>
ENDTEXT
...
ENDTEXT
		...
		FOR m.xj = ASC('A') TO ASC('Z')
			...
			IF _TALLY > 0
				...
				SCAN
					...
					m.lcHtml = m.lcHtml + CRLF + [<tr class="] + IIF(MOD(RECNO('vs'), 2) = 0, 'even', 'uneven') + ["><td class="w20"><a href="] + m.lcFunction + [.html">] + m.lcFunction + [</a></td><td>] + m.lcDescription + '</td></tr>'
				ENDSCAN	
	...
	FUNCTION CreateFunctionsByCategoryPage
		...
		SCAN
				...
				m.lcHtml = m.lcHtml + [<tr class="] + IIF(MOD(RECNO('vg'), 2) = 0, 'even', 'uneven') + ["><td><a href="group] + vg.groupid + [.html">] + HtmlChars(m.lcGroup) + [</a></td><td>] + HtmlChars(vg.descrip) + [</td></tr>] + CRLF
		ENDSCAN	
	...
	FUNCTION CreateCategoryPages
		...
		SCAN
			...
			SCAN
				...
				m.lcDescription = THIS.CreateLinks(m.lcDescription, .T., m.lcFunction)
				m.lcDescription = THIS.CreateLinks(m.lcDescription, .F., m.lcFunction)
				m.lcHtml = m.lcHtml + CRLF + [<tr class="] + IIF(MOD(RECNO('vs'), 2) = 0, 'even', 'uneven') + ["><td><a href="] + m.lcFunction + [.html">] + m.lcFunction + [</a></td><td>] + m.lcDescription + '</td></tr>'
			ENDSCAN

			m.lcHtml = m.lcHtml + CRLF + '</table>' + CRLF	
			m.lcHtml = m.lcHtml + THIS.HtmlFooter()
			
			STRTOFILE(m.lcHtml, THIS.cHtmlPath + 'group' + gr.groupid + '.html', 0)
