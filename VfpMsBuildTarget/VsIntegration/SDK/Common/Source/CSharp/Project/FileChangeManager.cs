...
namespace Microsoft.VisualStudio.Package
{
	...
	internal class FileChangeManager : IVsFileChangeEvents
	{
			...
			if (this.FileChangedOnDisk != null)
			{
				for (int i = 0; i < numberOfFilesChanged; i++)
				{
					...
					if (this.observedItems.ContainsKey(fullFileName))
					{
						...
						this.FileChangedOnDisk(this, new FileChangedOnDiskEventArgs(fullFileName, info.ItemID));
					}
				}
			}
		...
		internal void ObserveItem(string fileName, uint id)
		{
			...
			{
				throw new ArgumentException(SR.GetString(SR.InvalidParameter), "id");
			}
			#endregion
			...
			if (!this.observedItems.ContainsKey(fullFileName))
			{
				...
				ErrorHandler.ThrowOnFailure(this.fileChangeService.AdviseFileChange(fullFileName, (uint)_VSFILECHANGEFLAGS.VSFILECHG_Time, this, out fileChangeCookie));
		...
		internal void StopObservingItem(string fileName)
		{
			...
			Debug.Assert(this.observedItems.ContainsKey(fullFileName), "Not observing file: " + fullFileName);
			#endregion
