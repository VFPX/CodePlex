...
namespace Microsoft.VisualStudio.Package
{
    ...
    public abstract class ProjectPackage : Microsoft.VisualStudio.Shell.Package
    {
        ...
        public ProjectTrustLevel GetProjectTrustLevel(Guid projectGuid)
        {
            ...
            if (!this.projectTrustTable.ContainsKey(projectGuid))
            {
                return ProjectTrustLevel.Unknown;
            }

            return projectTrustTable[projectGuid]; ;
        }

        /// <summary>
        /// Set the ProjectTrustLevel for a project
        /// </summary>
        /// <param name="projectGuid">the project instance guid associated with project where project trus level is set</param>
        ...
        public void SetProjectTrustLevel(Guid projectGuid, ProjectTrustLevel projectTrustLevel)
        {
            if (Guid.Empty == projectGuid)
            {
                 throw new ArgumentException(SR.GetString(SR.ParameterCannotBeNullOrEmpty, CultureInfo.CurrentUICulture), "projectGuid");
            ...
            if (this.projectTrustTable.ContainsKey(projectGuid))
            {
                this.projectTrustTable[projectGuid] = projectTrustLevel;
            }
            else
            {
                this.projectTrustTable.Add(projectGuid, projectTrustLevel);
            }
        }
